...
(function () {
  ...
  if (typeof wizardNumbered !== undefined && wizardNumbered !== null) {
    ...
    */
    ...
    $(".step1_customer_type").on("change", function(e){
      if($(this).attr("id") == "step1_customer_1"){
        $(".step1_disable_possible").attr("disabled", false);

        $("#step1_customer_name_select").addClass("d-none");
        //$('#step1_customer_name_select').next(".select2-container").hide();

        $("#step1_customer_name").removeClass("d-none");
      } else {
        $(".step1_disable_possible").attr("disabled", true);

        $("#step1_customer_name_select").removeClass("d-none");
        //$('#step1_customer_name_select').next(".select2-container").show();

        $("#step1_customer_name").addClass("d-none");
      }
    });

    $(".other_check").on("click", function(){
      var other_check_memo = $(this).closest(".other_check_group").find(".other_check_memo");
      if($(this).is(':checked')) {
        other_check_memo.attr("disabled", false);
      } else {
        other_check_memo.attr("disabled", true);
      }
    });
  }
